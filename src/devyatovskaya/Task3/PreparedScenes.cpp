...
void PreparedScenes::init() {
  ...
  scenes.push_back(default_scene_sphere(
      ShaderCollection::shaders["phong_wo_normal_mapping"]));
  scenes.push_back(default_scene_sphere(
      ShaderCollection::shaders["guro_wo_normal_mapping"]));

  scenes.push_back(
      default_scene_cube(ShaderCollection::shaders["phong_wo_normal_mapping"]));
  scenes.push_back(
      default_scene_cube(ShaderCollection::shaders["guro_wo_normal_mapping"]));
...
GLScene PreparedScenes::default_scene_sphere(ShaderData &data) {
  ...
  scene.add_object(data, generator, {-distance, 0, -distance}, {0, 0, 0},
                   material);
  scene.add_object(data, generator, {-distance, 0, distance}, {0, 0, 0},
                   material);
  scene.add_object(data, generator, {distance, 0, distance}, {0, 0, 0},
  ...
  const auto &light_object_generator =
      MeshGeneratorCollection::generators["sphere"];

  const auto &light_sphere_gen =
      std::dynamic_pointer_cast<GLSphereMeshGenerator>(light_object_generator);
  light_sphere_gen->radius_ = 0.2f;

  const auto light =
      std::make_shared<GLPointLight>(QVector3D{0, 0, 0}, QColor{185, 255, 208});
