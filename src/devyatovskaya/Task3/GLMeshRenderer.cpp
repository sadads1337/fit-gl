...
GLMeshRenderer::GLMeshRenderer(GLMesh &mesh, GLTransform &transform,
    ...
    : vbo_{QOpenGLBuffer::VertexBuffer}, ibo_{QOpenGLBuffer::IndexBuffer},
      mesh_{mesh}, transform_{transform}, material_{material} {}

void GLMeshRenderer::init_renderer(
...
void GLMeshRenderer::set_shader(
    std::shared_ptr<QOpenGLShaderProgram> shader_program) {
  shader_program_ = std::move(shader_program);
}
...
void GLMeshRenderer::upload_camera_details(const GLCamera &camera) const {
  ...
  shader_program_->setUniformValue(
      "normal_matrix", transform_.transform_.inverted().transposed());
...
void GLMeshRenderer::enable_attributes() const {
  ...
  shader_program_->setAttributeBuffer("posAttr", GL_FLOAT, 0, 3,
                                      sizeof(GLVertex));

  shader_program_->enableAttributeArray("normalAttr");
  shader_program_->setAttributeBuffer(
      "normalAttr", GL_FLOAT, offsetof(GLVertex, normal), 3, sizeof(GLVertex));

  shader_program_->enableAttributeArray("colAttr");
  shader_program_->setAttributeBuffer(
      "colAttr", GL_FLOAT, offsetof(GLVertex, color), 3, sizeof(GLVertex));

  shader_program_->enableAttributeArray("textureAttr");
  shader_program_->setAttributeBuffer("textureAttr", GL_FLOAT,
                                      offsetof(GLVertex, texture), 2,
                                      sizeof(GLVertex));

  shader_program_->enableAttributeArray("tangentAttr");
  shader_program_->setAttributeBuffer("tangentAttr", GL_FLOAT,
                                      offsetof(GLVertex, tangent), 3,
                                      sizeof(GLVertex));

  shader_program_->enableAttributeArray("bitangentAttr");
  shader_program_->setAttributeBuffer("bitangentAttr", GL_FLOAT,
                                      offsetof(GLVertex, bitangent), 3,
                                      sizeof(GLVertex));
}
...
void GLMeshRenderer::init_vbo() {
  ...
  vbo_.allocate(mesh_.vertices.data(),
                static_cast<int>(mesh_.vertices.size() * sizeof(GLVertex)));
}
...
void GLMeshRenderer::init_ibo() {
  ...
  ibo_.allocate(mesh_.indices.data(),
                static_cast<int>(mesh_.indices.size() * sizeof(unsigned)));
}
