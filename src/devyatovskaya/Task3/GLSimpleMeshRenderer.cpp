...
GLSimpleMeshRenderer::GLSimpleMeshRenderer(GLMesh &mesh, GLTransform &transform,
                                           GLMaterial &material)
    : GLMeshRenderer{mesh, transform, material} {}

void GLSimpleMeshRenderer::render(
    QOpenGLFunctions_3_0 &functions, const GLCamera &camera,
    const std::map<light_sptr, obj_sptr> &lights) {
...
void GLSimpleMeshRenderer::upload_lights_details(
    const std::map<light_sptr, obj_sptr> &lights) const {
  ...
  shader_program_->setUniformValue("directed_lightsCount",
                                   static_cast<int>(directed_lights));
  shader_program_->setUniformValue("point_lightsCount",
                                   static_cast<int>(point_lights));
  shader_program_->setUniformValue("spot_lightsCount",
                                   static_cast<int>(spot_lights));
}

void GLSimpleMeshRenderer::upload_texture_details(
    QOpenGLFunctions_3_0 &functions) {
  if (!material_.texture.empty()) {
