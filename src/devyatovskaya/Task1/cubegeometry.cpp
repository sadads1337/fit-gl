...
void CubeGeometry::setVertices() {
  ...
  for (auto i = 0u; i < sideSurfCount * dim;
       i += dim) { /* side surface vertices */
  ...
  for (auto i = sideSurfCount * dim; i < topIdxEnd * dim;
       i += dim) { /* top vertices */
  ...
  for (auto i = topIdxEnd * dim; i < bottomIdxEnd * dim;
       i += dim) { /* bottom vertices */
