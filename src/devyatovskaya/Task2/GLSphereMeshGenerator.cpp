...
GLSphereMeshGenerator::GLSphereMeshGenerator(const float radius,
                                             const std::size_t sector_count,
                                             const std::size_t stack_count)
    : radius_{radius}, sector_count_{sector_count}, stack_count_{stack_count} {}
...
void GLSphereMeshGenerator::init_vertices(std::vector<GLVertex> &vertices,
  ...
  for (auto i = 0u; i <= stack_count_; ++i) {
    ...
    for (auto j = 0u; j <= sector_count_; ++j) {
      ...
      vertex.texture = {static_cast<float>(j) / sector_count_,
                        static_cast<float>(i) / stack_count_};
  ...
  for (auto i = 0u; i < stack_count_; ++i) {
    ...
    for (auto j = 0u; j < sector_count_; ++j, ++k1, ++k2) {
      ...
      if (i != (stack_count_ - 1)) {
        indices.push_back(static_cast<unsigned>(k1 + 1u));
        indices.push_back(static_cast<unsigned>(k2));
        indices.push_back(static_cast<unsigned>(k2 + 1u));
      }
    }
  }
}
