...
void PhongLightingWidget::show_render_dialog() { render_dialog_.show(); }
...
void PhongLightingWidget::set_ambient_color(const QColor &color) {
  std::for_each(current_scene_.objects.begin(), current_scene_.objects.end(),
...
void PhongLightingWidget::catch_fps(const QString &fps) { emit send_fps(fps); }

PhongLightingWidget::PhongLightingWidget(QWidget *parent)
    : QOpenGLWidget{parent}, render_dialog_{*this}, lighting_dialog_{*this},
      morphing_dialog_{*this} {
  setFocusPolicy(Qt::StrongFocus);
  setMouseTracking(true);
  connect(&fps_counter_, &FPSCounter::emit_fps, this,
          &PhongLightingWidget::catch_fps);

  connect(&light_color_dialog_, &QColorDialog::currentColorChanged, this,
          &PhongLightingWidget::set_light_color);
  connect(&ambient_color_dialog_, &QColorDialog::currentColorChanged, this,
          &PhongLightingWidget::set_ambient_color);
  connect(&diffuse_color_dialog_, &QColorDialog::currentColorChanged, this,
          &PhongLightingWidget::set_diffuse_color);
  connect(&specular_color_dialog_, &QColorDialog::currentColorChanged, this,
          &PhongLightingWidget::set_specular_color);
...
void PhongLightingWidget::keyPressEvent(QKeyEvent *event) {
  if (event->key() == Qt::Key_1) {
    ...
    const auto current_scene =
        std::next(prep_scenes_.scenes.begin(), scene_count_);
...
void PhongLightingWidget::show_shininess_dialog() { lighting_dialog_.show(); }
...
void PhongLightingWidget::show_morphing_widget() { morphing_dialog_.show(); }
...
void PhongLightingWidget::paintGL() {
  ...
  std::for_each(current_scene_.objects.begin(), current_scene_.objects.end(),
                [&](std::shared_ptr<GLObject> &object) {
                  ...
                  object->renderer->shader_program_->setUniformValue(
                      "morphFactor", morph_factor_);
                });
...
void PhongLightingWidget::update_framerate() { fps_counter_.calculate_fps(); }
