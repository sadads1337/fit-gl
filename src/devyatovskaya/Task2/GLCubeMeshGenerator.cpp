...
GLMesh GLCubeMeshGenerator::generate(const QColor &color) const {
  ...
  for (auto i = 1u; i < number_of_faces; ++i) {
    std::copy(front_face.indices.begin(), front_face.indices.end(),
              std::back_inserter(cube_indices));

    std::for_each(cube_indices.begin() + i * front_face.indices.size(),
                  cube_indices.end(), [&](auto &index) {
                    index +=
                        static_cast<unsigned>(i * front_face.vertices.size());
                  });
  }
...
void GLCubeMeshGenerator::rotate_face(std::vector<GLVertex> &plane_vertices,
                                      const QMatrix4x4 &matrix) const {
  std::for_each(plane_vertices.begin(), plane_vertices.end(),
                [&](GLVertex &vertex) {
...
void GLCubeMeshGenerator::get_side_faces(std::vector<GLVertex> &plane_vertices,
  ...
  for (auto i = 0u; i < side_faces - 1; ++i) {
    ...
    std::copy(plane_vertices.begin(), plane_vertices.end(),
              std::back_inserter(cube_vertices));
  }
}

void GLCubeMeshGenerator::get_top_n_bottom(
    std::vector<GLVertex> &plane_vertices, std::vector<GLVertex> &cube_vertices,
    QMatrix4x4 &matrix) const {
...
void GLCubeMeshGenerator::init_indices([
    [maybe_unused]] std::vector<unsigned> &indices) const {}

void GLCubeMeshGenerator::init_vertices(
    [[maybe_unused]] std::vector<GLVertex> &vertices,
    [[maybe_unused]] const QColor &color) const {}
