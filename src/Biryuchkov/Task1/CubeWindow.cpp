...
constexpr std::array<QVector3D, 16u> vertices = {
...
void CubeWindow::render() {
  ...
  glVertexAttribPointer(posAttr_, 3, GL_FLOAT, GL_FALSE, sizeof(QVector3D),
                        nullptr);
  glEnableVertexAttribArray(posAttr_);

  array_buffer_.bind();
  index_buffer_.bind();
  glDrawElements(GL_TRIANGLE_STRIP, static_cast<GLsizei>(indicies.size()),
