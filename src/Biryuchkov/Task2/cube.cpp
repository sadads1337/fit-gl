...
Cube::Cube(size_t steps, QVector3D const &size, QColor const &face_color,
...
void Cube::initialize(const size_t steps, QColor const &face_color,
...
void Cube::setEdgeColor(const QColor &color, const int face_idx) {
  if (face_idx == -1) {
    for (auto &face : faces_) {
      face.setEdgeColor(color);
    }
  } else {
    Q_ASSERT(face_idx >= 0 && face_idx < 6);
    faces_[face_idx].setEdgeColor(color);
  }
}

QColor Cube::getFaceColor(const int face_idx) {
  Q_ASSERT(face_idx >= 0 && face_idx < 6);
  return faces_[face_idx].getFaceColor();
}

QColor Cube::getEdgeColor(const int face_idx) {
  Q_ASSERT(face_idx >= 0 && face_idx < 6);
  return faces_[face_idx].getEdgeColor();
}

void Cube::setFaceColor(const QColor &color, const int face_idx) {
  if (face_idx == -1) {
    for (auto &face : faces_) {
    ...
    Q_ASSERT(face_idx >= 0 && face_idx < 6);
    faces_[face_idx].setFaceColor(color);
  }
}
