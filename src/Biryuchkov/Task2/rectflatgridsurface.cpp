...
rectFlatGridSurface::rectFlatGridSurface(size_t steps, const QColor &face_color,
                                         const QColor &edge_color)
    : arrayBuf_(QOpenGLBuffer::VertexBuffer),
      indexBuf_(QOpenGLBuffer::IndexBuffer), face_color_(face_color),
...
rectFlatGridSurface::rectFlatGridSurface(const rectFlatGridSurface &other,
                                         const QMatrix4x4 &transformation)
    : vertices_(other.vertices_), indices_(other.indices_),
      indexBuf_(QOpenGLBuffer::IndexBuffer), face_color_(other.face_color_),
...
void rectFlatGridSurface::initialize(size_t steps) {
  ...
  for (size_t j = 0; j < steps; ++j) {
    for (size_t i = 0; i < steps; ++i) {
      ...
      auto p = i + j * (steps + 1);
      indices_.push_back(static_cast<int>(k));
      indices_.push_back(static_cast<int>(p));
      indices_.push_back(static_cast<int>(p + 1));

      indices_.push_back(static_cast<int>(p + 1));
      indices_.push_back(static_cast<int>(k + 1));
      indices_.push_back(static_cast<int>(k));
    }
  }
}
...
QColor rectFlatGridSurface::getFaceColor() const { return face_color_; }

QColor rectFlatGridSurface::getEdgeColor() const { return edge_color_; }
...
rectFlatGridSurface
rectFlatGridSurface::getTransformated(const QMatrix4x4 &matrix) {
  auto copy = *this;
...
void rectFlatGridSurface::load() {
  arrayBuf_.bind();
  indexBuf_.bind();
  arrayBuf_.allocate(vertices_.data(),
                     static_cast<int>(vertices_.size()) * sizeof(QVector3D));
  indexBuf_.allocate(indices_.data(),
                     static_cast<int>(indices_.size()) * sizeof(GLushort));
}
