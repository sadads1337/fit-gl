...
constexpr auto LIGHT_INTENSITY = 1.5F;
...
bool intersect(const Ray &ray, const std::vector<Sphere> &spheres,
               const std::vector<Plane> &planes, QVector3D &hit,
               QVector3D &normal, Material &material, const QImage &texture) {
...
QVector3D cast_ray(const Ray &ray, const std::vector<Sphere> &spheres,
                   const std::vector<Plane> &planes, const QImage texture,
                   size_t depth = 0) {
  QVector3D point;
  QVector3D normal;
  Material material;
  ...
  if (depth > MAX_REFLECTIONS ||
      !intersect(ray, spheres, planes, point, normal, material, texture)) {
  ...
  QVector3D reflect_color = cast_ray(Ray{reflect_origin, reflect_direction},
                                     spheres, planes, texture, depth + 1);
  ...
  QVector3D shadow_origin = QVector3D::dotProduct(light_direction, normal) < 0
                                ? point - normal * 1e-3f
                                : point + normal * 1e-3f;
  ...
  if (!intersect(Ray{shadow_origin, light_direction}, spheres, planes,
                 shadow_point, shadow_normal, matter, texture) &&
  ...
  const auto result_color =
      ...
      specular_light_intensity * LIGHT_COLOR * material.albedo.y() +
...
int main(int argc, char *argv[]) {
  ...
  parser.process(app);
  if (parser.isSet(FilterOption) && parser.isSet(WidthOption) &&
      parser.isSet(HeightOption)) {
  ...
  Material empty{};
  Material ivory{QVector3D(0.1f, 0.5f, 0.6f), 50., 1.0,
                 QVector4D(0.6f, 0.3f, 0.1f, 0.0f)};
  Material rubber{QVector3D(0.3f, 0.1f, 0.3f), 10., 1.0,
                  QVector4D(0.9f, 0.1f, 0.0f, 0.0f)};
  Material mirror{QVector3D(1.0f, 1.0f, 1.0f), 1425., 1.0,
                  QVector4D(0.0f, 1.0f, 0.8f, 0.0f)};
  Material glass{QVector3D(0.7f, 0.8f, 0.8f), 125., 1.5,
  ...
  for (auto y = 0; y < result.height(); y++) {
    for (auto x = 0; x < result.width(); x++) {
      ...
      const auto dir_z = -result.height() / (2.f * std::tan(PI / 6.f));
      Ray ray{CAMERA_POS, QVector3D(dir_x, dir_y, dir_z).normalized()};
      res = cast_ray(ray, spheres, planes, texture);
      result.setPixelColor(x, y,
                           qRgb(std::min(int(res.x() * 255), 255),
                                std::min(int(res.y() * 255), 255),
                                std::min(int(res.z() * 255), 255)));
    }
  }
  if (filter == "nearest") {
  ...
  if (filter == "linear") {
    result.scaled(WIDTH, HEIGHT, Qt::IgnoreAspectRatio,
