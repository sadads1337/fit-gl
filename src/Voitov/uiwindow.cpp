...
void UIWindow::init()
{
    ...
    vertexesBuf.allocate(this->figures->getVertexes().data(),
                 static_cast<std::int32_t>(this->figures->getVertexes().size() * sizeof(QVector3D)));

    indexesBuf.allocate(this->figures->getIndexes().data(),
                 static_cast<std::int32_t>(this->figures->getIndexes().size() * sizeof(GLint)));

    std::cout << this->figures->getIndexes().size()<<" " << this->figures->getVertexes().size()<< std::endl;
...
void UIWindow::render()
{
    ...
    matrix.rotate(100.0 * frame_ / screen()->refreshRate(), this->figures->getRotationAxis());//rotate around axis (010)
    ...
    glDrawElements(GL_TRIANGLES, static_cast<GLsizei>(this->figures->getIndexes().size()), GL_UNSIGNED_INT, nullptr);
