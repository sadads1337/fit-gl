  ...
  for (const auto &o : objects) {
    ...
    if (rec.has_value()) {
      ...
      QVector3D refract_dir =
          refract(r.direction(), rec->normal, rec->material.refractiveIndex)
              .normalized();
      QVector3D reflect_orig =
          QVector3D::dotProduct(reflect_dir, rec->normal) < 0
              ? rec->p - rec->normal * 1e-3f
              : rec->p + rec->normal * 1e-3f;
      QVector3D refract_orig =
          QVector3D::dotProduct(refract_dir, rec->normal) < 0
              ? rec->p - rec->normal * 1e-3f
              : rec->p + rec->normal * 1e-3f;
...
int main(int argc, char *argv[]) {
  ...
  objects.emplace_back(std::make_shared<Sphere>(
      QVector3D(-2.2f, 0.5f, 0.0f), 0.5, ivory, QVector3D(1.0f, 1.0f, 0.0f)));
  objects.emplace_back(std::make_shared<Sphere>(
      QVector3D(1.2f, 0.5f, 0.0f), 0.5, rubber, QVector3D(0.1f, 0.1f, 0.3f)));
  objects.emplace_back(std::make_shared<Sphere>(
