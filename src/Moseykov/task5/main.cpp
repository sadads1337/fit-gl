...
constexpr QVector3D BACKGROUND_COLOR{0.0F, 0.7F, 1.0F};
constexpr QVector3D LIGHT_COLOR{1.0F, 1.0F, 1.0F};
constexpr QVector3D LIGHT_POS{0.65F, 5.0F, 6.0F};
constexpr QVector3D CAMERA_POS{0.65F, 1.0F, 3.0F};
...
QVector3D trace(const Ray &r,
  ...
  QVector3D ambient_coefficient = AMBIENT_COLOR * AMBIENT_STRENGTH;
  for (const auto &o : objects) {
    ...
    if (rec.has_value()) {
      ...
      QVector3D refract_dir =
          refract(r.direction(), rec->norm, rec->material.refractiveIndex)
              .normalized();
      QVector3D reflect_orig = QVector3D::dotProduct(reflect_dir, rec->norm) < 0
                                   ? rec->p - rec->norm * 1e-3f
                                   : rec->p + rec->norm * 1e-3f;
      QVector3D refract_orig = QVector3D::dotProduct(refract_dir, rec->norm) < 0
                                   ? rec->p - rec->norm * 1e-3f
                                   : rec->p + rec->norm * 1e-3f;
      QVector3D reflect_color =
          trace(Ray(reflect_orig, reflect_dir), objects, reflection_depth - 1);
      QVector3D refract_color =
          trace(Ray(refract_orig, refract_dir), objects, reflection_depth - 1);
      QVector3D shadow_orig = QVector3D::dotProduct(light_dir, rec->norm) < 0
                                  ? rec->p - rec->norm * 1e-3f
                                  : rec->p + rec->norm * 1e-3f;
      const auto shadow_rec = o->hit(Ray(shadow_orig, light_dir));
      if (!(shadow_rec.has_value()) || shadow_rec->t > light_distance) {
        diffuse_light_intensity =
            std::max(0.f, QVector3D::dotProduct(light_dir, rec->norm));
        specular_light_intensity =
            std::pow(std::max(0.f, QVector3D::dotProduct(
                                       reflect(light_dir, rec->norm),
                                       r.direction())),
                     rec->material.specularFactor) *
            LIGHT_INTENSITY;
      }
      auto k = std::exp(-(rec->p - r.origin()).length() / 100.f);
      auto resultColor =
          ambient_coefficient * rec->color +
          rec->color * diffuse_light_intensity * rec->material.albedo.x() +
          LIGHT_COLOR * specular_light_intensity * rec->color *
              rec->material.albedo.y() +
          reflect_color * rec->color * rec->material.albedo.z() +
          refract_color * rec->color * rec->material.albedo.w();
...
int main(int argc, char *argv[]) {
  ...
  parser.setApplicationDescription("Params");
  ...
  QCommandLineOption WidthOption(
      ...
      QCoreApplication::translate("main", "width value"));
  parser.addOption(WidthOption);
  QCommandLineOption HeightOption(
      QStringList() << "height"
                    << "height-value",
      QCoreApplication::translate("main", "height"),
      QCoreApplication::translate("main", "height value"));
  ...
  objects.emplace_back(std::make_shared<Sphere>(
      QVector3D(0.0f, 0.5f, 0.0f), 0.5, mirror, QVector3D(1.0f, 1.0f, 1.0f)));
  objects.emplace_back(std::make_shared<Sphere>(
      QVector3D(0.0f, 1.5f, 1.5f), 0.3, rubber, QVector3D(1.0f, 1.0f, 0.0f)));
  objects.emplace_back(std::make_shared<Sphere>(
      QVector3D(1.3f, 1.5f, 1.5f), 0.3, ivory, QVector3D(0.1f, 0.1f, 0.3f)));
  objects.emplace_back(std::make_shared<Sphere>(
      QVector3D(1.1f, 0.5f, 0.0f), 0.5, glass, QVector3D(0.6f, 0.7f, 0.8f)));
  objects.emplace_back(
      std::make_shared<Floor>(QVector3D(), QVector3D(0, 1, 0)));
  ...
  for (int y = 0; y < result.height(); ++y) {
    for (int x = 0; x < result.width(); ++x) {
      ...
      auto dir_z = -result.height() / (2.f * std::tan(3.141592F / 6.f));
      Ray ray = Ray(CAMERA_POS, QVector3D(dir_x, dir_y, dir_z).normalized());
      QVector3D color = trace(ray, objects, MAX_REFLECTIONS);
