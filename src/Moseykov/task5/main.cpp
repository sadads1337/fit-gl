...
QVector3D trace(const Ray& r, const std::vector<std::shared_ptr<Hittable>> &objects, int reflection_depth) {
    if(reflection_depth == 0) { return BACKGROUND_COLOR; }
    ...
    for(const auto &o : objects){
        ...
        if(rec.has_value()){
            ...
            QVector3D refract_dir = refract(r.direction(), rec->norm, rec->material.refractiveIndex).normalized();
            QVector3D reflect_orig = QVector3D::dotProduct(reflect_dir,rec->norm) < 0 ? rec->p - rec->norm*1e-3f : rec->p + rec->norm*1e-3f;
            QVector3D refract_orig = QVector3D::dotProduct(refract_dir, rec->norm) < 0 ? rec->p - rec->norm*1e-3f : rec->p + rec->norm*1e-3f;
            ...
            QVector3D shadow_orig = QVector3D::dotProduct(light_dir,rec->norm) < 0 ? rec->p - rec->norm*1e-3f : rec->p + rec->norm*1e-3f;
            const auto  shadow_rec = o->hit(Ray(shadow_orig, light_dir));
            if(!(shadow_rec.has_value()) || shadow_rec->t > light_distance){
                diffuse_light_intensity = std::max(0.f, QVector3D::dotProduct(light_dir,rec->norm));
                specular_light_intensity = std::pow(std::max(0.f, QVector3D::dotProduct(reflect(light_dir, rec->norm),r.direction())), rec->material.specularFactor)*LIGHT_INTENSITY;
            }
            auto k = std::exp(-(rec->p-r.origin()).length()/100.f);
            auto resultColor = ambient_coefficient*rec->color + rec->color * diffuse_light_intensity*rec->material.albedo.x()
...
int main(int argc, char *argv[]){
    ...
    objects.emplace_back(std::make_shared<Sphere>(QVector3D(0.0f,1.5f,1.5f), 0.3, rubber, QVector3D(1.0f, 1.0f, 0.0f)));
    objects.emplace_back(std::make_shared<Sphere>(QVector3D(1.3f,1.5f,1.5f), 0.3, ivory,QVector3D  (0.1f, 0.1f, 0.3f)));
    objects.emplace_back(std::make_shared<Sphere>(QVector3D(1.1f,0.5f,0.0f), 0.5, glass, QVector3D(0.6f, 0.7f, 0.8f)));
    objects.emplace_back(std::make_shared<Floor>(QVector3D(), QVector3D(0, 1, 0)));
