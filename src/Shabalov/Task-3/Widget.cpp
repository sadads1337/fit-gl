...
void Widget::initCube(const float width, const int N) {
  ...
  for (auto z = -1; z <= 1; z += 2) {
    for (auto y = 0; y < N; y++) {
      for (auto x = 0; x < N; x++) {
        vertexes.emplace_back(QVector3D(z * (-half_width + x * part_width),
                                        -half_width + y * part_width,
                                        z * half_width),
  ...
  for (auto x = -1; x <= 1; x += 2) {
    for (auto z = 0; z < N; z++) {
      for (auto y = 0; y < N; y++) {
        vertexes.emplace_back(QVector3D(x * half_width,
                                        -half_width + y * part_width,
  ...
  for (auto y = -1; y <= 1; y += 2) {
    for (auto x = 0; x < N; x++) {
      for (auto z = 0; z < N; z++) {
        vertexes.emplace_back(QVector3D(-half_width + x * part_width,
                                        y * half_width,
...
void Widget::setXRotation(const float angle) {
  x_rot = angle / 100.0f;
  update();
}

void Widget::setYRotation(const float angle) {
  y_rot = angle / 100.0f;
  update();
}

void Widget::setZRotation(const float angle) {
  z_rot = angle / 100.0f;
  update();
}
