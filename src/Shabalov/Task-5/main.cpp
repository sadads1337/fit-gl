...
[[nodiscard]] auto mix(const QVector3D &a, const QVector3D &b, const float k) {
  return k * a + (1 - k) * b;
}

[[nodiscard]] auto reflect(const QVector3D &incident, const QVector3D &normal) {
  return incident - 2.f * normal * (QVector3D::dotProduct(incident, normal));
}

[[nodiscard]] auto refract(const QVector3D &incident, const QVector3D &normal,
...
[[nodiscard]] auto scene_intersect(const Ray &ray,
                                   const std::vector<Sphere> &spheres,
                                   const std::vector<Plane> &planes,
                                   QVector3D &hit, QVector3D &normal,
                                   Material &material) {
  ...
  for (const auto &sphere : spheres) {
    ...
    if (sphere.ray_intersect(ray, dist_i) && dist_i < spheres_dist) {
      ...
      if (sphere.material.isTexture()) {
        int u = (sphere.material.image.width() - 1) *
                (1 + atan2(normal.z(), normal.x()) / PI) * 0.5f;
        int v = (sphere.material.image.height() - 1) * acosf(normal.y()) / PI;
        QColor tmp = sphere.material.image.pixel(u, v);
        material.diffuseColor =
            QVector3D(tmp.redF(), tmp.greenF(), tmp.blueF());
      } else {
...
[[nodiscard]] auto cast_ray(const Ray &ray, const std::vector<Sphere> &spheres,
                            const std::vector<Plane> &planes,
  ...
  if (depth > MAX_REFLECTIONS ||
      !scene_intersect(ray, spheres, planes, point, normal, material)) {
    return BACKGROUND_COLOR;
  }
  const auto reflect_direction = reflect(ray.direction, normal).normalized();
  const auto refract_direction =
      refract(ray.direction, normal, material.refractiveIndex).normalized();
  const auto reflect_origin =
      QVector3D::dotProduct(reflect_direction, normal) < 0.f
          ? point - normal * offset
          : point + normal * offset;
  const auto refract_origin =
      QVector3D::dotProduct(refract_direction, normal) < 0.f
          ? point - normal * offset
          : point + normal * offset;
  Ray reflect_ray = {reflect_origin, reflect_direction};
  Ray refract_ray = {refract_origin, refract_direction};
  const auto reflect_color =
      cast_ray(reflect_ray, spheres, planes, lights, depth + 1);
  const auto refract_color =
      cast_ray(refract_ray, spheres, planes, lights, depth + 1);
  ...
  for (auto light : lights) {
    ...
    const auto shadow_origin =
        QVector3D::dotProduct(light_direction, normal) < 0.f
            ? point - normal * offset
            : point + normal * offset;
    QVector3D shadow_point;
    QVector3D shadow_normal;
    Material tmpmaterial;
    Ray light_ray = {shadow_origin, light_direction};
    if (scene_intersect(light_ray, spheres, planes, shadow_point, shadow_normal,
                        tmpmaterial) &&
  ...
  const auto obj_color = material.diffuseColor * diffuse_light_intensity *
                             material.proportion.x() +
                         QVector3D(1., 1., 1.) * specular_light_intensity *
                             material.proportion.y() +
...
int main() {
  Material ivory(QVector3D(0.4f, 0.4f, 0.3f), 50., 1.0,
                 QVector4D(0.6f, 0.3f, 0.1f, 0.0f));
  Material rubber(QVector3D(0.1f, 0.1f, 0.3f), 10., 1.0,
                  QVector4D(0.9f, 0.1f, 0.0f, 0.0f));
  Material mirror(QVector3D(1.0f, 1.0f, 1.0f), 1425., 1.0,
                  QVector4D(0.0f, 1.0f, 0.8f, 0.0f));
  Material glass(QVector3D(0.6f, 0.7f, 0.8f), 125., 1.5,
                 QVector4D(0.0f, 0.5f, 0.1f, 0.8f));
  ...
  Light light_1 = {QVector3D(30, 50, -25), 1.5f};
  Light light_2 = {QVector3D(30, 20, 30), 2.0f};
  Light light_3 = {QVector3D(0, 20, 0), 2.5f};
